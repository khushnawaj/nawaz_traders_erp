      ...
      <div className="glass-modal rounded-3xl max-w-lg w-full shadow-2xl overflow-hidden my-8 border border-slate-200/80 dark:border-slate-800 animate-in fade-in zoom-in duration-200 text-slate-900 dark:text-white">
        ...
        <div className="bg-gradient-to-r from-emerald-800 via-emerald-900 to-slate-900 text-white p-4 sm:p-5 flex items-center justify-between border-b border-emerald-700/40">
          <div>
            <h3 className="font-semibold text-base sm:text-lg flex items-center gap-2">
              ...
              {initialData ? 'Edit Warehouse / Godown' : 'Add Godown / Storage Shed'}
            </h3>
            <p className="text-xs text-emerald-200/80 font-normal mt-0.5">Grain Storage Shed & Warehouse Registry</p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-5 sm:p-6 space-y-4 max-h-[80vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div className="sm:col-span-2">
              <label className="app-label">
                <Warehouse className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" /> Warehouse Name *
              </label>
              <input
                type="text"
                required
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="app-label">
                <MapPin className="w-3.5 h-3.5 text-amber-500" /> Location / Address
              </label>
              <input
                type="text"
            ...
            <div>
              <label className="app-label">
                <Scale className="w-3.5 h-3.5 text-emerald-500" /> Storage Capacity (MT)
              </label>
              <div className="relative">
                ...
                />
                <span className="absolute right-3.5 top-2.5 text-xs font-semibold text-slate-400">MT</span>
              </div>
            </div>
          </div>
          ...
          <div>
            <label className="app-label">
              <User className="w-3.5 h-3.5 text-slate-500" /> In-Charge / Supervisor
            </label>
            <input
              type="text"
          ...
          <div className="pt-3 flex items-center justify-end gap-3 border-t border-slate-200 dark:border-slate-800">
            ...
            >
              {loading ? 'Saving...' : initialData ? 'Update Godown' : 'Save Godown'}
            </button>
          </div>
        </form>
      </div>
